...
A `.streamlit/config.toml` a **8510** portot fixálja. Egyszerre csak egy Streamlit app fusson ebből a mappából (vagy állítsd le a másik alkalmazást).
...
A projektben a **`.streamlit/config.toml`** beállítja a **fix 8510** portot (ha a `photo_sorter` mappából indítod).

```bash
cd photo_sorter
source .venv/bin/activate
python3 -m streamlit run organizer_metal_app.py
```

Böngésző: **http://127.0.0.1:8510**
